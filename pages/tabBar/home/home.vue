<template>
	<view>
		<view class="swiper" style="background-color: #F7F3E8;">
			<view style="width:100%;">
				<image style="width: 100%;height:100;" src="/static/img/home/header.png"></image>
			</view>
			<view
				style="position:relative;bottom:52upx;background-color: #F7F3E8;border-top-left-radius: 26upx;padding: 40upx;padding-bottom:0;">
				<view>
					<view class="center-cut-menu">
						<scroll-view show-scrollbar="false" scroll-x="true" scroll-with-animation="true"
							class="scroll-view" :scroll-left="scrollLeft">
							<view class="scroll-item" v-for="(item, index) in list" :key="index"
								@click="changeMenu(index)">
								<view class="bar" :class="curIndex == index? 'on-bar' : ''">
									<view>{{item.name}}</view>
									<view class="line" style="width:30%;margin:0 auto; border:2px solid #000;"></view>
								</view>
							</view>
						</scroll-view>
					</view>
				</view>
			</view>
		</view>
		<view id = "waterfall" style = "margin-top:-68upx;">
			<WaterfallsFlow :wfList='productList' @itemTap="toGoods" />
		</view>
		<!-- <view class="loading-text">{{ loadingText }}</view> -->
	</view>
</template>

<script>
	import WaterfallsFlow from '../../../components/waterfall/WaterfallsFlow/WaterfallsFlow.vue'
	import home from "@/api/home.js"
	export default {
		components:{
			WaterfallsFlow
		},
		data() {
			return {
				contentScrollW: 0, // 导航区宽度
				curIndex: 0, // 当前选中
				scrollLeft: 0, // 横向滚动条位置
				list: [],
				//猜你喜欢列表
				productList: [],
				loadingText: '正在加载...',
				max:0
			};
		},
		onPageScroll(e) {
			//兼容iOS端下拉时顶部漂移
			this.headerPosition = e.scrollTop >= 0 ? "fixed" : "absolute";
			this.headerTop = e.scrollTop >= 0 ? null : 0;
			this.statusTop = e.scrollTop >= 0 ? null : -this.statusHeight + 'px';
		},
		//下拉刷新，需要自己在page.json文件中配置开启页面下拉刷新 "enablePullDownRefresh": true
		onPullDownRefresh() {
			setTimeout(function() {
				uni.stopPullDownRefresh();
			}, 1000);
		},
		//上拉加载，需要自己在page.json文件中配置"onReachBottomDistance"
		onReachBottom() {
			let len = this.productList.length;
			console.log(len)
			if (len >= this.max) {
				this.loadingText = '到底了';
				return false;
			}
			uni.showToast({
				title: '加载中'
			});
			this.fetchItem()
		},
		onLoad() {
			
		},
		mounted() {
			// 获取标题区域宽度，和每个子元素节点的宽度
			this.getScrollW()
			this.fetchClass()
			this.fetchItem()
		},
		created() {
			this.fetchClass()
			// this.fetchClass()
		},
		methods: {
			fetchClass(){
				home.classify().then(res => {
					this.list = res.data.data
				})
			},
			fetchItem(){
				home.item({classify_id:this.curIndex + 1}).then(res => {
					this.productList = this.productList.concat(res.data.data.data)
					this.max = res.data.data.count_all
				})
			},
			// test(){
			// 	const data = abc.decrypt("5235080773f7cc59f5ad28d88ef91df6d04de6809147c5f46b32538e89857835f61ac88281800169efb2d56cd026a2049f373419b613cfee25ed45790070179a")
			// 	console.log(data)
			// },
			// 获取标题区域宽度，和每个子元素节点的宽度以及元素距离左边栏的距离
			getScrollW() {
				let query = uni.createSelectorQuery().in(this);
				query.select('.scroll-view').boundingClientRect(data => {
					// 拿到 scroll-view 组件宽度
					this.contentScrollW = data.width
				}).exec();

				query.selectAll('.scroll-item').boundingClientRect(data => {
					let dataLen = data.length;
					for (let i = 0; i < dataLen; i++) {
						//  scroll-view 子元素组件距离左边栏的距离
						this.list[i].left = data[i].left;
						//  scroll-view 子元素组件宽度
						this.list[i].width = data[i].width
					}
				}).exec()
			},

			// 选择标题
			changeMenu(index) {
				this.curIndex = index;
				this.productList = []
				this.fetchItem()
				this.scrollLeft = this.list[index].left - this.contentScrollW / 2 + this.list[index].width / 2;
			},

			//商品跳转
			toGoods(e) {
				// uni.showToast({
				// 	title: '商品' + e.id,
				// 	icon: 'none'
				// });
				uni.navigateTo({
					url: '/pages/goods/goods?id='+e.id
				});
			},
		}
	};
</script>
<style lang="scss">
	.clear {
		clear: both;
	}
	
	.uni-progress-info{
		font-size: 20upx;
	}
	
	.uni-progress-inner-bar{
		border-radius: 200upx;
	}
	
	#waterfall{
		background-color: #F9F3E7;
	}

	.center-cut-menu {
		width: 100%;
		height: 100rpx;
		box-sizing: border-box;

		.scroll-view {
			white-space: nowrap;

			.scroll-item {
				display: inline-block;
				text-align: center;
			}
		}
	}

	scroll-view ::-webkit-scrollbar {
		display: none !important;
		width: 0 !important;
		height: 0 !important;
		-webkit-appearance: none;
		background: transparent;
	}

	page {
		position: relative;
		background-color: #F7F3E8;
	}

	@font-face {
		font-family: 'HMfont-home';
		src: url('data:application/x-font-woff2;charset=utf-8;base64,') format('woff2');
	}

	.header {
		width: 92%;
		padding: 0 4%;
		height: 100upx;
		display: flex;
		align-items: center;
		position: fixed;
		top: 0;
		z-index: 10;
		background-color: #F7F3E8;

		/*  #ifdef  APP-PLUS  */
		top: var(--status-bar-height);
		/*  #endif  */


	}

	.place {
		background-color: #ffffff;
		height: 100upx;
		/*  #ifdef  APP-PLUS  */
		margin-top: var(--status-bar-height);
		/*  #endif  */
	}

	.swiper {
		width: 100%;
		justify-content: center;

		.bar {
			float: left;
			height: 48upx;
			margin-left: 32upx;

			/** 文本1 */
			font-size: 28upx;
			font-weight: 400;
			letter-spacing: 0px;
			line-height: 48upx;
			color: rgba(159, 147, 128, 1);
			text-align: left;
			vertical-align: top;

			.line {
				display: none;
			}
		}

		.on-bar {
			height: 48upx;

			/** 文本1 */
			font-size: 32upx;
			font-weight: 400;
			letter-spacing: 0px;
			line-height: 48upx;
			color: rgba(74, 58, 53, 1);
			text-align: left;
			vertical-align: top;

			.line {
				display: block;
			}
		}
	}
	
	.loading-text {
		width: 100%;
		display: flex;
		justify-content: center;
		align-items: center;
		height: 60upx;
		color: #979797;
		font-size: 24upx;
		text-align: center;
	}
</style>